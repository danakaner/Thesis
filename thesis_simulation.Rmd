...
compute_test_predictions_under_settings <- function(n, error_distribution_type, error_distribution_params,
  ...
  test_predictions = data.frame(matrix(NA, nrow = n_test*(length(all_taus)), ncol = (sample_size + 3)))

  test_predictions_bias_correction = data.frame(matrix(NA, nrow = n_test*(length(all_taus)), ncol = (sample_size + 3)))
  ...
  for (sample in seq(1,sample_size)) {
    ...
    for (i in 0:(length(all_taus)-1)) {
         ...
         test_predictions_bias_correction[((n_test*(i))+1):(n_test*(i+1)),3] <- rep("with_correction",n_test)
         test_predictions_bias_correction[((n_test*(i))+1):(n_test*(i+1)),(sample+3)] <- (step_mean_test/n_folds)
...
results = data.frame(wrong_tau = NA, dist = NA, right = NA, left = NA)
...
for (wrong_tau in seq(0.5, true_tau, by = 0.005)) {
  ...
  right <- exp(quan_wrong^2 - quan_true^2)
  left <- mult_true/mult_wrong
