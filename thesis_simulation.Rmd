...
## parallel
packages = c("dplyr", "quantreg", "isotone", "sqldf", "knitr", "reshape", "reshape2","caret", "EXRQ","evd")
```
...
### attantion: altered it after running the simulatio, check before i run it again
...
compute_test_predictions_under_settings <- function(n, error_distribution_type, error_distribution_params,
  ...
  test_predictions_EXRQ = data.frame(matrix(NA, nrow = n_test, ncol = (sample_size + 3)))

  test_predictions_ICQR = data.frame(matrix(NA, nrow = n_test*(length(all_taus)), 
                                                       ncol = (sample_size + 3)))
  ...
  for (sample in seq(1,sample_size)) {
   ...
   ## EVT two-steps estimation - EXRQ
   k <- 4.5 * (n^(1/3))
   wang_pred <- TwoStage(train_data$y, as.matrix(train_data %>% select(-y)), 
                         as.matrix(test_data%>% select(-y)), tau_true, k = k, tol = 1e-04)
   
   test_predictions_EXRQ[1:n_test,1] <- rep(all_taus[(i+1)],n_test)
   test_predictions_EXRQ[1:n_test,2] <- true_quan_line_on_test
   test_predictions_EXRQ[1:n_test,3] <- rep("EXRQ",n_test)
   test_predictions_EXRQ[1:n_test,(sample+3)] <- wang_pred$Q2Stage
    
    ## less extrem quantile and ICQR
    n_folds <- 5
    ...
    for (i in 0:(length(all_taus)-1)) {
         ...
         test_predictions_ICQR[((n_test*(i))+1):(n_test*(i+1)),1] <- rep(all_taus[(i+1)],n_test)
         test_predictions_ICQR[((n_test*(i))+1):(n_test*(i+1)),2] <- true_quan_line_on_test
         test_predictions_ICQR[((n_test*(i))+1):(n_test*(i+1)),3] <- rep("ICQR",n_test)
         test_predictions_ICQR[((n_test*(i))+1):(n_test*(i+1)),(sample+3)] <- (step_mean_test/n_folds)
  ...
  colnames(test_predictions_EXRQ)[3] <- "method"
  colnames(test_predictions_EXRQ)[2] <- "true_quantile_in_x"
  colnames(test_predictions_EXRQ)[1] <- "quantile"
  colnames(test_predictions_ICQR)[3] <- "method"
  colnames(test_predictions_ICQR)[2] <- "true_quantile_in_x"
  colnames(test_predictions_ICQR)[1] <- "quantile"
  

  return (rbind(test_predictions,test_predictions_EXRQ,test_predictions_ICQR))
...
compute_Bias_Var <- function(test_predictions, n_test) {
  ...
  for (i in 1:(length(all_taus)*2+1)){
...
```{r}
write.csv(errors_normal, "cv_isotonic_and_exrq_outputs/errors_normal.csv", row.names=FALSE)
write.csv(errors_weibull, "cv_isotonic_and_exrq_outputs/errors_weibull.csv", row.names=FALSE)
write.csv(errors_t, "cv_isotonic_and_exrq_outputs/errors_t.csv", row.names=FALSE)
...
```{r}
errors_normal <- read.csv("cv_isotonic_and_exrq_outputs/errors_normal.csv", sep =",")

errors_t <- read.csv("cv_isotonic_and_exrq_outputs/errors_t.csv", sep =",")

errors_weibull <- read.csv("cv_isotonic_and_exrq_outputs/errors_weibull.csv", sep =",")
```


```{r}
filter_to_model_shape <- function(data, power, rho, melt_num = 8 ) {
  filtered_data <- data %>% 
  dplyr::filter((power==power) & (rho==rho)) %>% 
  melt(id.vars = seq(1,melt_num)) %>% 
  mutate(quantile_final = 
           ifelse(method != "no_correction", paste0(quantile,method),quantile)) %>% 
  select(-c(true_quan))
...
```{r}
normal_location_shift_linear_model <- 
  filter_to_model_shape(errors_normal, 1,0)

normal_scale_location_linear_model <- 
  filter_to_model_shape(errors_normal, 1,1)

normal_location_shift_quadratic_model <- 
  filter_to_model_shape(errors_normal, 2,0)

normal_scale_location_quadratic_model <- 
  filter_to_model_shape(errors_normal, 2,1)
```


```{r}
t_location_shift_linear_model <- 
    filter_to_model_shape(errors_t, 1,0)

t_scale_location_linear_model <- 
  filter_to_model_shape(errors_t, 1,1)

t_location_shift_quadratic_model <- 
  filter_to_model_shape(errors_t, 2,0)

t_scale_location_quadratic_model <- 
  filter_to_model_shape(errors_t, 2,1)
```


```{r}
weibull_location_shift_linear_model <- 
    filter_to_model_shape(errors_weibull, 1,0, melt_num = 9)

weibull_scale_location_linear_model <- 
  filter_to_model_shape(errors_weibull, 1,1, melt_num = 9)

weibull_location_shift_quadratic_model <- 
  filter_to_model_shape(errors_weibull, 2,0, melt_num = 9)

weibull_scale_location_quadratic_model <- 
  filter_to_model_shape(errors_weibull, 2,1, melt_num = 9)
```
...
ui <- fluidPage(
  ...
  verticalLayout(
    ...
    splitLayout(
        ...
        checkboxGroupInput("method", h5("Methods to show"), 
                    choices = list("EXRQ" = "EXRQ",
                                   "ICQR" = "ICQR",
                                   "No Correction" = "no_correction"),
                    selected = list("ICQR","no_correction")),
...
server <- function(input, output) {
  ...
  distribution_data <- reactive({
        ...
        if (input$distribution=='norm') {
          if (input$rho==0){
            if (input$power==1){
               chosen_distribution <- normal_location_shift_linear_model
            } else {
               chosen_distribution <- normal_location_shift_quadratic_model
            }
          } else {
            if (input$power==1){
               chosen_distribution <- normal_scale_location_linear_model
            } else {
               chosen_distribution <- normal_scale_location_quadratic_model
          ...
          if (input$rho==0){
            if (input$power==1){
            chosen_distribution <- t_location_shift_linear_model
            } else {
             chosen_distribution <- t_location_shift_quadratic_model

            }
          } else {
            if (input$power==1){
            chosen_distribution <- t_scale_location_linear_model
            } else {
            chosen_distribution <- t_scale_location_quadratic_model

            }
          }
        } else if (input$rho==0) {
          if (input$power==1){
          chosen_distribution <- weibull_location_shift_linear_model 
          } else {
          chosen_distribution <- weibull_location_shift_quadratic_model 

          }
        } else {
           if (input$power==1){
          chosen_distribution <- weibull_scale_location_linear_model 
           } else {
             chosen_distribution <- weibull_scale_location_quadratic_model 
   ...
   filtered_data_by_params <- reactive({
    ...
    method_data <- reactive({
        ...
        filtere_data_by_param <- 
          filtere_data_by_param %>% 
          dplyr::filter(method %in% input$method )
   ...
   # })
    ...
    output$boxplot <- renderPlot({
        ...
        scale_colour_manual(values = c("0.97" = "#318ef9",
                                       "0.97ICQR" = "#66adff",
                                       "0.98" = "#60cbf7",
                                       "0.98ICQR" = "#a3e4ff",
                                       "0.99" = "#ffb24f",
                                       "0.99ICQR" = "#ffc375"))+
        scale_fill_manual(values = c("0.97" = "#318ef9",
                                       "0.97ICQR" = "#66adff",
                                       "0.98" = "#60cbf7",
                                       "0.98ICQR" = "#a3e4ff",
                                       "0.99" = "#ffb24f",
...
```{r}
t_scale_location_linear_model %>% 
  filter(p==15 & method =="EXRQ" & (quantile==0.99)) %>% 
  group_by(n,power,rho,df) %>% 
  filter(variable =="MSE") %>% 
...
```{r, message=FALSE}
ggplot(normal_location_shift_linear_model %>% 
       dplyr::filter((p %in% c(15)) & (sigma == 0.1)  
                     & (quantile %in% c(0.98,0.99))
                     & (n %in% ns) & (power == 1) & (rho==1) 
                     & (method != "bias_correction_one") & (variable == "MSE") ),
       aes(x = as.factor(n), y = log(value+1), 
           fill = as.factor(quantile_final)))+
geom_boxplot(inherit.aes = T, outlier.size = 0.1, alpha = 0.7)+
...
ggplot(t_location_shift_linear_model %>% 
       dplyr::filter((p %in% c(15)) & (df %in% dfs)  
                     & (quantile %in% c(0.97,0.99))
                     & (n %in% ns) & (power == 1) & (rho==0) 
                     & (method != "bias_correction_one")),
       aes(x = as.factor(n), y = log(value+1), 
           fill = as.factor(quantile_final)))+
geom_boxplot(inherit.aes = T, outlier.size = 0.1, alpha = 0.7)+
...
error_distribution_type <- "weibull"
error_distribution_params <- c(shape,scale)
```


```{r}
test_data <- randomize_data(n_test, error_distribution_type = error_distribution_type, error_distribution_params = error_distribution_params, x_mean = x_mean, x_sd = x_sd, p=p, rho=rho, power=1)


train_data <- randomize_data(n, error_distribution_type, error_distribution_params, 
                             x_mean, x_sd, p=p, rho = rho, power=1) 

quan_line_coefficients <- compute_conditional_quantile_line(tau_true, error_distribution_type, error_distribution_params, rho = rho, p=p)

test_predictions <-
  compute_test_predictions_under_settings(n, error_distribution_type = "t",
                                          error_distribution_params = df,
                                          n_test = n_test, test_data = test_data, p=p,
                                          x_mean = x_mean, x_sd = x_sd, 
                                          rho = rho, sample_size = 1)
...
wang_pred <- TwoStage(train_data$y, 
         as.matrix(train_data %>% select(-y)), 
         as.matrix(test_data%>% select(-y)), tau.e = c(0.99), 
         k=4.5*(length(train_data$y)^0.3), tol = 1e-04)
sum(is.na(wang_pred$Q2Stage))

pred_99 <- 
  test_predictions %>% 
  filter((quantile == 0.99) & (method == "no_correction"))  %>% 
  select('X4')

pred_97 <- 
  test_predictions %>% 
  filter((quantile == 0.97) & (method == "no_correction"))  %>% 
  select('X4')

bias_correction_97 <- 
  test_predictions %>% 
  filter((quantile == 0.97) & (method == "bias_correction_one"))  %>% 
  select('X4')

true_quantile_in_x <- 
  test_predictions %>% 
  filter((quantile == 0.97) & (method == "bias_correction_one"))  %>% 
  select('true_quantile_in_x')

predictions_compare <- 
  test_data %>% 
  mutate(pred_99 = pred_99$X4,
         pred_97 = pred_97$X4,
         bias_correction_97 = bias_correction_97$X4,
         true_quantile_in_x = true_quantile_in_x$true_quantile_in_x,
         wang_pred = wang_pred$Q2Stage)

mean((predictions_compare$true_quantile_in_x - predictions_compare$wang_pred)^2, na.rm = T)
mean((predictions_compare$true_quantile_in_x - predictions_compare$bias_correction_97)^2)
mean((predictions_compare$true_quantile_in_x - predictions_compare$pred_99)^2)
mean((predictions_compare$true_quantile_in_x - predictions_compare$pred_97)^2)
