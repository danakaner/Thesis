...
compute_test_predictions_under_settings <- function(n, error_distribution_type, error_distribution_params,
  ...
  for (sample in seq(1,sample_size)) {
    ...
    for (i in 0:(length(all_taus)-1)) {
         ...
         step_mean_test <- rep(0, length(test_data$y))
         for (b in 1:bs) {
            new_train_data <- sample_n(train_data, len, replace = T)
            
            small_train_data <- sample_n(new_train_data, (bias_correction_val_size*len), replace = F)
            small_eval_data <- sqldf("select * from new_train_data except select * from small_train_data")
         ...
         test_predictions_bagged_isotonic[((n_test*(i))+1):(n_test*(i+1)),(sample+3)] <- (step_mean_test/bs)
...
```{r}
errors_normal <- read.csv("outputs_all/errors_normal.csv", sep =",")
```
...
```{r}
# errors_weibull <- read.csv("bagged_isotonic_outputs/outputs/errors_weibull.csv", sep =",")
```
...
```{r}
errors_t <- read.csv("outputs_all/errors_t.csv", sep =",")
```
