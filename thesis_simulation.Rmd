...
errors_weibull <- read.csv("relative_and_cv_corrections_outputs/errors_weibull.csv", sep =",")
```
...
filter_to_model_shape <- function(data, power, rho, melt_num = 8 ) {
  ...
  mutate(quantile_final = 
           ifelse(method == "bias_correction_one", paste0(quantile,"_correction_1"),
...
```{r}
unique(normal_location_shift_linear_model$quantile_final)
```
...
wang_pred <- TwoStage(train_data$y, 
         ...
         as.matrix(test_data%>% select(-y)), 0.99, k=10, tol = 1e-04)
