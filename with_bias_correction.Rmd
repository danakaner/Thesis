...
randomize_data <- function(n, error_distribution_type, error_distribution_params, 
                           x_mean, x_sd, true_coefs_x, rho = 0) {
...
compute_conditional_quantile_line <- function(tau_true, error_distribution_type, 
                                              error_distribution_params, true_coefs_x, rho = 0) {
...
compute_test_predictions_under_settings <- function(n, error_distribution_type, error_distribution_params,
                                                n_test, test_data, x_mean, x_sd, rho = 0, 
  ...
  compute_conditional_quantile_line(tau_true, error_distribution_type, 
                                    error_distribution_params, true_coefs_x, rho = rho)
...
```{r}
## model settings
true_coefs_xs <- list(rep(1,2), rep(1,9) , rep(1,16))
rhos <- c(0,1)
...
# 
# for (true_coefs_x in true_coefs_xs) {
#    for (rho in rhos) {
# 
#     for (sigma in sigmas) {
# 
...
#                                 true_coefs_x = true_coefs_x, rho = rho)
# 
#     for (n in ns) {
#       
#          
#          cat("p: " ,length(true_coefs_x)-1, "rho: ", rho, "sigma: ", sigma, ", n:", n, "\n")
# 
#          test_predictions <-
...
#   
#          errors_normal <- rbind(errors_normal, data.frame("p" = length(true_coefs_x)-1,
#                                                           "rho"= rho,
#                                                           "n" = n,
#                                                           "sigma" = sigma,
#                                                           errors))
    ...
    for (shape in shapes) {
      for (scale in scales) {
        ...
        test_data <- randomize_data(n_test, error_distribution_type = "weibull",
                      ...
                      x_mean = x_mean, x_sd = x_sd, true_coefs_x = true_coefs_x,rho = rho)

        for (n in ns) {

            cat("p: " ,length(true_coefs_x)-1, "rho: ", rho,  "shape: ", 
            ...
            errors_weibull <- rbind(errors_weibull,
                                    data.frame("p" = length(true_coefs_x)-1,
                                               "rho"= rho,
                                               "n" = n,
                                               "shape" = shape,
                                               "scale" = scale,
  ...
  for (rho in rhos) {
    for (df in dfs) {
        ...
        for (n in ns) {
          ...
          cat( "p: " ,length(true_coefs_x)-1, "rho: ", rho,  "df: ", df, ", n:", n, "\n")
            ...
            test_predictions <-
              compute_test_predictions_under_settings(n, error_distribution_type = "t",
                                                      ...
                                                      n_test = n_test, test_data = test_data,
                                                      x_mean = x_mean, x_sd = x_sd, rho = rho,
                                                      bias_correction_val_size = 0.7, sample = 300)
  ...
  verticalLayout(
    ...
    splitLayout(
        ...
        checkboxGroupInput("n", h5("Train set size"),
                       choices = c(125,250,500,1000), selected = c(125,250,500,1000)))),
  ...
  ## distribution parameters
    sidebarPanel(
            ...
            radioButtons("sigma", h5("Sigma"),
                 choices = c(0.001, 0.01, 0.1), selected = 0.001),
            radioButtons("df", h5("Degreed of freedom"),
                 choices = c(1,3,10), selected = 1),
            
            splitLayout(

            radioButtons("shape", h5("Shape"),
                 choices = c(0.5,1,1.5), selected = 1),
...
server <- function(input, output) {
  ...
  distribution_data <- reactive({
   ...
   distribution_func_data <- reactive({
     ...
     output$density <- renderPlot({
        ...
        dens <- 
          ggplot(distribution_func_data(), aes(x)) +
        geom_density(col = "cornflowerblue", fill = "cornflowerblue", alpha = 0.7) +
        labs(x = "residual value", y = "density") +
        theme_grey() 
        
        hist <-
          ggplot(distribution_func_data(), aes(x)) +
        geom_histogram(col = "cornflowerblue", fill = "cornflowerblue", alpha = 0.7, bins = 100) +
        labs(x = "residual value", y = "count") +
        theme_grey() 
        
        grid.arrange(dens, hist, ncol = 2)
     })
...
ggplot(normal_melted_data_iid %>% 
       dplyr::filter((p %in% c(1,5,10,20)) & (sigma == 0.1) & (rho==0) 
                     & (quantile %in% c(0.93, 0.95,0.97,0.99))
                     & (n %in% c(125,250,500,1000,2000)) 
                     & (variable == "MSE")),
       aes(x = as.factor(n), y = log(value+1), fill = as.factor(quantile)))+
geom_boxplot(inherit.aes = T, 
             col = "#585858", outlier.size = 0.15)+
facet_grid(~p, scales = "free") + 
labs(x = "n", y = "log(value+1)", 
     title = "MSE for normal distribution", subtitle = "Over a range of p") +
scale_fill_brewer(palette="Paired") +
 # theme_light() +
theme_grey() +
theme(legend.position = "bottom")
...
ggplot(normal_melted_data_iid %>% 
       dplyr::filter((p %in% c(1)) & (sigma %in% c(0.001,0.1,1)) & (rho==0) 
                     & (quantile %in% c(0.93, 0.95,0.97,0.99))
                     & (n %in% c(125,250,500,1000,2000))),
       aes(x = as.factor(n), y = log(value+1), fill = as.factor(quantile)))+
geom_boxplot(inherit.aes = T, 
             col = "#585858", outlier.size = 0.15)+
facet_grid(sigma~variable, scales = "free") + 
labs(x = "n", y = "log(value+1)", 
     title = "MSE for normal iid errors", subtitle = "Over a range of p",
     fill = "Quantile") +
scale_fill_brewer(palette="Paired") +
theme_grey() +
theme(legend.position = "bottom")
...
ggplot(normal_melted_data_het %>% 
       dplyr::filter((p %in% c(1)) & (sigma %in% c(0.001,0.1,1)) & (rho==1) 
                     & (quantile %in% c(0.93, 0.95,0.97,0.99))
                     & (n %in% c(125,250,500,1000,2000))),
       aes(x = as.factor(n), y = log(value+1), fill = as.factor(quantile)))+
geom_boxplot(inherit.aes = T, 
...
facet_grid(sigma~variable, scales = "free") + 
labs(x = "n", y = "log(value+1)", 
     title = "MSE for normal hetrogenous errors", subtitle = "Over a range of p",
     fill = "Quantile") +
scale_fill_brewer(palette="Paired") +
theme_grey() +
theme(legend.position = "bottom")
